...
formHeader("Form:Clinical Service Note");
...
function writeRow($label, $field, $disabled=false) {
	...
	echo "<tr><td align=\"left\" class=\"forms\">&nbsp;&nbsp;".xlt($label)."</td>";
 	echo "<td colspan=\"3\"><textarea name=\"$field\" rows=\"4\" cols=\"60\" wrap=\"virtual name\" ".($disabled ? 'disabled="disabled"' : '').">";
 	if (isset($obj{$field})) {
 		echo text($obj{$field});
 	}
 	echo "</textarea></td></tr>\n";
...
<body class="body_top">
	...
	<form method='post' name='my_form'
		<?php echo "action='$rootdir/forms/clinical_service_note/save.php?id=" . attr($formid) ."'"; ?>>

		<table border="0">
			<tr>
				<th colspan='4'><?php echo xlt('Clinical Service Note') ?></th>
			</tr>
			<tr>
